...
generate_notifications_list <-function(feedback_file,
                                       feedback_sheet,
                                       penji_file,
                                       start_date,
                                       outfile) {
...
# Read in courses served ----
  ...
  # Read in feedback spreadsheet ----
  
  ff_raw <- read_excel(feedback_file, # 'data/Feedback Details 11.3.2025.xlsx',
                       sheet = feedback_sheet) # 'Sheet 1 - Feedback Details (2)')
  ...
  ## Make one row of penji info for each student ----
  ...
  ## Merge feedback and penji data ----
  
  merged_df <- left_join(ff, penji_wide)
  
  # Filter to students noted for feedback for a course who have NOT used both services for that course ----
